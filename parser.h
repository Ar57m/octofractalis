...
// Structure to represent AST nodes
struct ASTNode {
    enum NodeType {
        CONSTANT,
        VARIABLE,
    ...
    void* data;
    using EvaluatorFunction = Quaternion (*)(const ASTNode* node);
    EvaluatorFunction evaluator;

    ASTNode(NodeType t, void* d, EvaluatorFunction evalFunc)
        : type(t), data(d), evaluator(evalFunc) {}

    Quaternion evaluate() const {
        return evaluator(this);
    }
};
...
struct BinaryFunctionNodeData {
    const ASTNode* operand1;
    const ASTNode* operand2;
    Quaternion (*func)(const Quaternion&, const Quaternion&);
};
...
Quaternion evaluateConstantNode(const ASTNode* node);
Quaternion evaluateVariableNode(const ASTNode* node);
Quaternion evaluateArrayAccessNode(const ASTNode* node);
Quaternion evaluateUnaryFunctionNode(const ASTNode* node);
Quaternion evaluateBinaryFunctionNode(const ASTNode* node);
Quaternion evaluateTernaryFunctionNode(const ASTNode* node);
...
Quaternion evaluateArrayAccessNode(const ASTNode* node) {
    ArrayAccessNodeData* data = static_cast<ArrayAccessNodeData*>(node->data);
    uint32_t evaluatedIndex = static_cast<uint32_t>(abs(data->indexNode->evaluate().real));
    return (evaluatedIndex < data->size) ? Quaternion(data->array[evaluatedIndex]) : Quaternion(0);
}
...
// Define buffer sizes for each allocator (tweak these as needed)
constexpr size_t AST_NODE_BUFFER_SIZE                   = 16384;
constexpr size_t CONSTANT_NODE_DATA_BUFFER_SIZE         = 4096;
constexpr size_t VARIABLE_NODE_DATA_BUFFER_SIZE         = 4096;
constexpr size_t ARRAY_ACCESS_NODE_DATA_BUFFER_SIZE     = 4096;
...
ASTNode* createVariableNode(
    ...
    VariableNodeData* data = dataAllocator.allocate();
    if (!data) return nullptr;
    data->valuePtr = valuePtr; // Store direct pointer
    ASTNode* node = allocator.allocate();
    if (!node) return nullptr;
    return new (node) ASTNode(ASTNode::VARIABLE, data, evaluateVariableNode);
}

ASTNode* createArrayAccessNode(
    StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& allocator, 
    StackAllocator<ArrayAccessNodeData, ARRAY_ACCESS_NODE_DATA_BUFFER_SIZE>& dataAllocator, 
    double* array, uint32_t size, ASTNode* indexNode
) {
...
ASTNode* createUnaryFunctionNode(
    StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& allocator, 
    StackAllocator<UnaryFunctionNodeData, UNARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& dataAllocator, 
    ASTNode* operand, Quaternion (*func)(const Quaternion&)
) {
...
ASTNode* createBinaryFunctionNode(
    StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& allocator, 
    StackAllocator<BinaryFunctionNodeData, BINARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& dataAllocator, 
    ASTNode* operand1, ASTNode* operand2, Quaternion (*func)(const Quaternion&, const Quaternion&)
) {
...
ASTNode* createTernaryFunctionNode(
    StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& allocator, 
    StackAllocator<TernaryFunctionNodeData, TERNARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& dataAllocator, 
    ASTNode* operand1, ASTNode* operand2, ASTNode* operand3, 
    Quaternion (*func)(const Quaternion&, const Quaternion&, const Quaternion&)
...
char* my_strncpy(char* dest, const char* src, size_t n) {
    char* ret = dest;
    // Copy characters from src to dest until we hit n characters or a null terminator.
    while (n && *src) {
        *dest++ = *src++;
        --n;
    }
    // If we haven't reached n characters, pad dest with null terminators.
    while (n--) {
        *dest++ = '\0';
    }
    return ret;
...
private:
    ...
    // Allocators for nodes and their data
    StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& nodeAllocator;
    StackAllocator<ConstantNodeData, CONSTANT_NODE_DATA_BUFFER_SIZE>& constantDataAllocator;
    StackAllocator<VariableNodeData, VARIABLE_NODE_DATA_BUFFER_SIZE>& variableDataAllocator;
    StackAllocator<ArrayAccessNodeData, ARRAY_ACCESS_NODE_DATA_BUFFER_SIZE>& arrayDataAllocator;
...
public:
    Parser(const char* expr, size_t expr_size,
        VariableEntry* vars, size_t numVars,
        ArrayEntry* arrays, size_t numArrays,
        StackAllocator<ASTNode, AST_NODE_BUFFER_SIZE>& nodeAlloc,
        StackAllocator<ConstantNodeData, CONSTANT_NODE_DATA_BUFFER_SIZE>& constAlloc,
        StackAllocator<VariableNodeData, VARIABLE_NODE_DATA_BUFFER_SIZE>& varAlloc,
        StackAllocator<ArrayAccessNodeData, ARRAY_ACCESS_NODE_DATA_BUFFER_SIZE>& arrAlloc,
        StackAllocator<UnaryFunctionNodeData, UNARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& unaryAlloc,
        StackAllocator<BinaryFunctionNodeData, BINARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& binaryAlloc,
        StackAllocator<TernaryFunctionNodeData, TERNARY_FUNCTION_NODE_DATA_BUFFER_SIZE>& ternaryAlloc)
        : expr_size(expr_size), pos(0), varEntries(vars), numVars(numVars),
        arrEntries(arrays), numArrays(numArrays),
        nodeAllocator(nodeAlloc), constantDataAllocator(constAlloc),
        variableDataAllocator(varAlloc), arrayDataAllocator(arrAlloc),
        unaryDataAllocator(unaryAlloc), binaryDataAllocator(binaryAlloc),
        ternaryDataAllocator(ternaryAlloc)
    {
        // Copy the provided string into our fixed-size buffer.
        my_strncpy(this->expr, expr, MAX_EXPR_SIZE);
...
private:
    ...
    static constexpr bool isImaginaryChar(const char c) {
        return (c > 'h' && c < 'l');
    }
    ...
    // Check if expr starting at pos matches token.
    bool startsWith(int posi, const char* token) {
        int i = 0;
        while (token[i] != '\0') {
            if (expr[posi + i] == '\0' || expr[posi + i] != token[i])
                return false;
    ...
    // Check that the token found at position pos is a standalone token.
    bool isTokenBoundary(int posi, int tokenLen) {
        // Left boundary: either pos==0 or previous char is not identifier.
        if (posi > 0 && isIdentifierChar(expr[posi - 1])) {
            return false;
        }
    ...
    // Shift the expression to the right by 'shift' characters starting from pos.
    // Returns false if there isn’t enough space.
    bool shiftRight(int posi, int shift) {
        int len = my_strlen(expr);
        if (len + shift >= MAX_EXPR_SIZE)
            return false;
        // Shift from the end (including '\0') to pos.
    ...
    // Shift the expression to the left by 'shift' characters starting from pos.
    void shiftLeft(int posi, int shift) {
        int len = my_strlen(expr);
        for (int i = posi; i <= len; i++) {
            expr[i - shift] = expr[i];
        }
    ...
    const VariableEntry* findVariable(const char* name) {
        for (size_t i = 0; i < numVars; ++i) {
            // Compare the stored name with the given name.
            if (my_strcmp(varEntries[i].name, name) == 0) {
                return &varEntries[i];
            }
        }
    ...
    // Find an array by name using a C‑string.
    const ArrayEntry* findArray(const char* name) {
        for (size_t i = 0; i < numArrays; ++i) {
            if (my_strcmp(arrEntries[i].name, name) == 0) {
                return &arrEntries[i];
            }
    ...
    void replaceTokens(const char* tokens[], int numTokens) {
        for (int t = 0; t < numTokens; t++) {
            const char* token = tokens[t];
            // Build the replacement string into rep.
            char rep[64];  // Sufficient for "(" + token + "+0.000001)"
            ...
            // Calculate token and replacement lengths.
            int tokenLen = 0;
            while (token[tokenLen] != '\0') {
                tokenLen++;
            }
            ...
            // Copy the temporary result back to expr.
            my_strncpy(expr, temp, MAX_EXPR_SIZE);
            expr[MAX_EXPR_SIZE - 1] = '\0'; // Ensure null termination.
        }
    }
    

    ASTNode* error_zero = createConstantNode(nodeAllocator,constantDataAllocator, Quaternion(0));
    ...
    ASTNode* parseVariableOrFunction() {
        char name[128] = {};
        size_t nameIndex = 0;
    ...
    ASTNode* parseNumber() {
        ...
        if (num_index == 0)
            my_strncpy(number, "0.0",sizeof(number));
        const double parsedValue = (identifier != '\0' && my_strcmp(number, "0.0") == 0) ? 1.0 : my_atof(number);
    ...
    // Main parseFunction, now much shorter:
    ASTNode* parseFunction(const char* func) {
        const unsigned int functionHash = str2int(func);
        ++pos;
        char old_expr[MAX_EXPR_SIZE];
        my_strncpy(old_expr, expr, MAX_EXPR_SIZE);
        size_t old_pos = pos;
    ...
    // Helper for unary functions:
    ASTNode* parseUnaryFunction(unsigned int hash, ASTNode* arg, const size_t old_pos) {
        switch (hash) {
            case str2int("sqrt"):
                return createUnaryFunctionNode(nodeAllocator, unaryDataAllocator, arg,
                    [](const Quaternion& a) { return a.sqrt(); });
            ...
            case str2int("airy"):
                return createUnaryFunctionNode(nodeAllocator, unaryDataAllocator, arg,
                    [](const Quaternion& a) { return a.airy(); });
            // Special case: 'diff' needs to modify the expression
            case str2int("diff"):
            {
                // Replace 'z' in the remainder of the expression:
                char old_expr[MAX_EXPR_SIZE];
                my_strncpy(old_expr, expr, MAX_EXPR_SIZE);
                size_t n_pos = pos;
                size_t old_expr_size = expr_size;
                ...
                pos = n_pos;
                expr_size = old_expr_size;
                my_strncpy(expr, old_expr, MAX_EXPR_SIZE);
    ...
    // Helper for binary functions:
    ASTNode* parseBinaryFunction(unsigned int hash, ASTNode* arg1, ASTNode* arg2) {
        switch (hash) {
            case str2int("logn"):
                return createBinaryFunctionNode(nodeAllocator, binaryDataAllocator,
                    arg1, arg2, [](const Quaternion& a, const Quaternion& b) { return a.logn(b); });
    ...
    // Helper for ternary functions:
    ASTNode* parseTernaryFunction(unsigned int hash, ASTNode* arg1, ASTNode* arg2, ASTNode* arg3) {
        switch (hash) {
            case str2int("If"):
                return createTernaryFunctionNode(nodeAllocator, ternaryDataAllocator,
                    arg1, arg2, arg3,
